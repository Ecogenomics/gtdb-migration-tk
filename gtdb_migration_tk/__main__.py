...
def __dir_suffix(group, required=False):
    defaults = ', '.join('{} for --db {}'.format(suffix, db)
                         for db, suffix in sorted(config.MARKER_DIR_SUFFIX.items()))
    group.add_argument('--dir_suffix', default=None, required=required,
                       help='Suffix of the marker directory and files written inside the prodigal directory of each genome. '
                            'Default: {} '
                            '(see MARKER_DIR_SUFFIX in config.py).'.format(defaults))
