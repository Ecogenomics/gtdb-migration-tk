...
def __folder_suffix(group, required):
    group.add_argument('--folder_suffix',
                       help='Suffix for pfam and tigrfam folder. ( example: 15.0,33.1,33.1_lite,15.0_lite)',
                       required=required)
...
def get_main_parser():
    ...
    with subparser(sub_parsers, 'top_hit', 'Generate TopHit file for Tigrfam or Pfam.') as parser:
        with arg_group(parser, 'required named arguments') as grp:
            ...
            __folder_suffix(grp, required=True)
        with arg_group(parser, 'options arguments') as grp:
            __silent(grp)
            __cpus(grp)
